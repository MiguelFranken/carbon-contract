 ...
 * @param owner Owner account that deployed the Carbon contract
...
async function _addWhitelistedContract(
  ...
  if (revert) {
    ...
    if (assert) {
      ...
      const whiteListed = await carbon.getListContract(id); // index starts with 0
...
export async function mint(
  ...
  const pTx = carbon.connect(minter).mint({
    value,
  });
 ...
 *
 * @param carbon Deployed Carbon contract
 * @param owner Owner of Carbon contract
...
export async function assertCollectedBalance(
  ...
  const balances = await carbon.bal();
...
export async function assertBalanceOf(
  ...
  const tx = carbon.queryBalance(listIndex, communityTokenHolder.address);
  await expect(tx).to.not.be.reverted;
...
export async function assertListContract(
  ...
  const tx = carbon.getListContract(lcId);
  if (exists) {
    await expect(tx).to.not.be.reverted;
  } else {
