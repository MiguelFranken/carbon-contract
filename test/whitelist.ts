...
describe("Whitelist", function () {
  ...
  it("should revert when trying to get a contract that does not exist", async () => {
    const tx = contracts.carbon.getListContract(0);
  ...
  describe("Add Contracts", function () {
    ...
    it("should reset percRoyal when removing contract", async () => {
      ...
      // remove contract
      await assertListContract(contracts.carbon, 0);
      const tx1 = contracts.carbon.connect(owner).removeWhitelisting(0);
      await expect(tx1).to.not.be.reverted;
      await assertListContract(contracts.carbon, 0, false);
  ...
  describe("Remove Contracts", function () {
    ...
    it("should be possible to remove a contract", async () => {
      await assertListContract(contracts.carbon, listContractId1);
      const tx1 = contracts.carbon
        .connect(owner)
        .removeWhitelisting(listContractId1);
      await expect(tx1).to.not.be.reverted;
      await assertListContract(contracts.carbon, listContractId1, false);

      await assertListContract(contracts.carbon, listContractId2);
      const tx2 = contracts.carbon
        .connect(owner)
        .removeWhitelisting(listContractId2);
      await expect(tx2).to.not.be.reverted;
      await assertListContract(contracts.carbon, listContractId2, false);
    });

    it("should be possible to remove a contract by owner only", async () => {
      await assertListContract(contracts.carbon, listContractId1);
      const tx = contracts.carbon
        .connect(nonOwner)
        .removeWhitelisting(listContractId1);
  ...
  describe("Query Balance", function () {
    beforeEach(async () => {
      await addWhitelistedContracts(contracts.carbon);
    });
